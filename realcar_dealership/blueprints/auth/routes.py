...
def signup():
    ...
    if request.method == 'POST' and registerform.validate_on_submit():
        ...
        if User.query.filter(User.username == username).first():
            flash(f"Username already exists. Please sign in", category='warning')
            return redirect('/signin')
        if User.query.filter(User.email == email).first():
            flash("Email already exists. Please sign in", category='warning')
            return redirect('/signin')
        ...
        flash(f"You have been successfully registered {username}", category='success')
        return redirect('/signin')
